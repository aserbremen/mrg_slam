...
public:
    ...
    bool flush(std::shared_ptr<GraphSLAM> &graph_slam, const std::vector<KeyFrame::Ptr> &keyframes, const std::unordered_map<ros::Time, KeyFrame::Ptr, RosTimeHash> &keyframe_hash, const ros::Time &latest_keyframe_stamp);

    const g2o::VertexPlane* floor_plane_node() const {
        return floor_plane_node_ptr;
    }
