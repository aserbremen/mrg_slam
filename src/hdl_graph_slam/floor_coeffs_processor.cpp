...
void FloorCoeffsProcessor::onInit(ros::NodeHandle &nh, ros::NodeHandle &mt_nh, ros::NodeHandle &private_nh_) {
 ...
 */
void FloorCoeffsProcessor::floor_coeffs_callback(const hdl_graph_slam::FloorCoeffsConstPtr& floor_coeffs_msg) {
 ...
 */
bool FloorCoeffsProcessor::flush(std::shared_ptr<GraphSLAM> &graph_slam, const std::vector<KeyFrame::Ptr> &keyframes, const std::unordered_map<ros::Time, KeyFrame::Ptr, RosTimeHash> &keyframe_hash, const ros::Time &latest_keyframe_stamp) {    
    std::lock_guard<std::mutex> lock(floor_coeffs_queue_mutex);
