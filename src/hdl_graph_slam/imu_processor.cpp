...
void ImuProcessor::onInit(ros::NodeHandle &nh, ros::NodeHandle &mt_nh, ros::NodeHandle &private_nh_) {
...
void ImuProcessor::imu_callback(const sensor_msgs::ImuPtr& imu_msg) {
...
bool ImuProcessor::flush(std::shared_ptr<GraphSLAM> &graph_slam, const std::vector<KeyFrame::Ptr> &keyframes, const std::string &base_frame_id) {
    std::lock_guard<std::mutex> lock(imu_queue_mutex);
