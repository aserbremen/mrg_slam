...
#include "mrg_slam_component.cpp"
...
// In terminal: ros2 run --prefix 'gdbserver localhost:3000' mrg_slam manual_composition
...
// ros2 run mrg_slam map2odom_publisher_ros2.py

int
main( int argc, char const *argv[] )
{
    if( argc < 2 ) {
        std::cout << "Usage: ros2 run mrg_slam mrg_slam_manual_composition <path_to_config_yaml>" << std::endl;
        return 1;
    }
    ...
    std::vector<rclcpp::Parameter> params_mrg_slam               = std::vector<rclcpp::Parameter>( params_shared );
    ...
    for( auto const &node_params : param_map ) {
        ...
        if( node_name.find( "mrg_slam_component" ) != std::string::npos ) {
            params_mrg_slam.insert( params_mrg_slam.end(), node_params.second.begin(), node_params.second.end() );
        }
    }
    ...
    auto prefiltering_component = std::make_shared<mrg_slam::PrefilteringComponent>( options, params_prefiltering );
    exec.add_node( prefiltering_component );
    auto floor_detection_component = std::make_shared<mrg_slam::FloorDetectionComponent>( options, params_floor_detection );
    exec.add_node( floor_detection_component );
    auto scan_matching_odometry_component = std::make_shared<mrg_slam::ScanMatchingOdometryComponent>( options,
                                                                                                       params_scan_matching_odometry );
    exec.add_node( scan_matching_odometry_component );
    auto mrg_slam_component = std::make_shared<mrg_slam::MrgSlamComponent>( options, params_mrg_slam );
    exec.add_node( mrg_slam_component );
