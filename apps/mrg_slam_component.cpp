...
private:
    ...
    void slam_pose_broadcast_callback( mrg_slam_msgs::msg::PoseWithName::ConstSharedPtr slam_pose_msg )
    {
        ...
        // Check if other_robot_name is registered in the SLAM experiment (para: multi_robot_names)
        if( request_graph_service_clients.find( other_robot_name ) == request_graph_service_clients.end() ) {
            RCLCPP_WARN_STREAM( this->get_logger(), "Trying to request graph from "
                                                        << other_robot_name
                                                        << " which is not registered as a request graph service client, returning..." );
            return;
        }
